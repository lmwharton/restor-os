"""Bootstrap: companies, users, jobs tables with RLS
...
def downgrade() -> None:
    ...
    op.execute("DELETE FROM storage.buckets WHERE id = 'photos'")
    ...
    op.execute('DROP POLICY IF EXISTS "users_select_company" ON users')
    ...
    op.execute("DROP TRIGGER IF EXISTS trg_prevent_admin_escalation ON users")
    ...
    op.execute("DROP FUNCTION IF EXISTS prevent_admin_self_escalation()")
    op.execute("DROP FUNCTION IF EXISTS update_updated_at()")
    op.execute("DROP FUNCTION IF EXISTS get_my_company_id()")
